...
**Addendum (post-implementation, impl-review F3)**: `supabase/migrations/20260724120000_grant_table_privileges_to_service_role.sql` was added but not originally planned. It grants `service_role` full privileges on all public tables/sequences (plus `ALTER DEFAULT PRIVILEGES`) — needed because a freshly-provisioned `supabase start` instance (used by Phase 7's CI E2E job) grants `service_role` no privileges on app-created tables, so the E2E seed fixture (which uses a service-role key to bypass onboarding UI) failed with "permission denied." Mirrors the existing `authenticated`-role grant pattern from `context/foundation/lessons.md`; `service_role` already bypasses RLS by design and is never exposed to end users.
