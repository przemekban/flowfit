...
|---|---|---|---|---|---|---|
| 1 | API + authorization safety net | Defend the highest-churn, highest-impact boundary: input validation and cross-user access control | #1, #2 | unit + integration | change opened | `context/changes/testing-api-authorization-safety-net/` |
...
- TBD — see §3 Phase 2 (service-layer unit tests for `profile.ts`/`plan.ts`, extending the `session.test.ts` mock pattern).

### 6.2 Adding an integration test

- TBD — see §3 Phase 1 (API-route validation/authorization integration tests).
...
- TBD — see §3 Phase 1 (this phase establishes the per-route contract-test pattern for `src/pages/api`).

### 6.5 Adding a test for cross-user authorization (IDOR)

- TBD — see §3 Phase 1 (Risk #2 response guidance).
