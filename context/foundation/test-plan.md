...
| #7   | The user resets their profile via the modal on `/onboarding`, the profile is deleted from Supabase, and they are successfully redirected back to the blank onboarding form.                               | "The reset button doesn't trigger deletion or fails silently"                                                                                                                       | `/onboarding` UI and the `/api/profile/reset` POST handler.                                                                                    | E2E test                                                                            | Mocking database response for reset (we want to verify actual Supabase deletion).                                                                                   |tput (flaky, costs quota).                     |
...
| 3   | AI generation safety net + upcoming route guard | Catch malformed AI output before persistence and future-proof the protected-route list ahead of workout-history shipping | #4, #6        | unit + integration | complete | `context/archive/2026-07-30-testing-ai-generation-safety-net-and-route-guard/` |
| 4   | Quality-gates wiring                            | Lock unit+integration and critical-flow e2e into CI as required gates                                                    | cross-cutting | gates              | complete | `context/archive/2026-07-30-quality-gates-wiring/`                             |
